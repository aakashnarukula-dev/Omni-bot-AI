...
object TimeParser {
    fun isTimeReply(input: String): Boolean = Regex("(?i)^\\s*(at\\b|on\\b|in\\b|tomorrow\\b|today\\b|every\\b|daily\\b|weekdays?\\b|weekends?\\b|next\\b|noon\\b|midnight\\b|monday\\b|tuesday\\b|wednesday\\b|thursday\\b|friday\\b|saturday\\b|sunday\\b|\\d)").containsMatchIn(input)
    fun parse(input: String, now: ZonedDateTime = ZonedDateTime.now()): ParsedTime? = runCatching {
        ...
        if (relative != null) {
            if (Regex("\\b(every|daily|weekdays?|weekends?|weekly|monthly|tomorrow|today)\\b|\\d\\s*(am|pm)\\b|\\d:\\d").containsMatchIn(text)) return null
        ...
        val weekdays = Regex("\\b(every weekday|weekdays)\\b").containsMatchIn(text)
        val weekends = Regex("\\b(every weekend|weekends)\\b").containsMatchIn(text)
        val repeatedWeekday = Regex("\\bevery\\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday)\\b")
            .find(text)?.groupValues?.get(1)?.uppercase()?.let(DayOfWeek::valueOf)
        val repeat = when {
            daily -> "daily"
            weekdays -> "weekdays"
            weekends -> "weekends"
            repeatedWeekday != null -> "weekly:${repeatedWeekday.name}"
            else -> "none"
        }
        if (Regex("\\b(every|weekly|monthly|yearly)\\b").containsMatchIn(text) && repeat == "none") return null
        ...
        val weekday = repeatedWeekday ?: DayOfWeek.entries.firstOrNull { Regex("\\b${it.name.lowercase()}\\b").containsMatchIn(text) }
        ...
        val date = when {
            ...
            repeatedWeekday != null -> now.toLocalDate().with(TemporalAdjusters.nextOrSame(repeatedWeekday))
            weekdays -> generateSequence(now.toLocalDate()) { it.plusDays(1) }
                .first { it.dayOfWeek !in setOf(DayOfWeek.SATURDAY, DayOfWeek.SUNDAY) }
            weekends -> generateSequence(now.toLocalDate()) { it.plusDays(1) }
                .first { it.dayOfWeek in setOf(DayOfWeek.SATURDAY, DayOfWeek.SUNDAY) }
        ...
        if (!target.isAfter(now)) {
            target = when {
                repeat.startsWith("weekly:") -> target.plusWeeks(1)
                weekdays -> generateSequence(target.plusDays(1)) { it.plusDays(1) }
                    .first { it.dayOfWeek !in setOf(DayOfWeek.SATURDAY, DayOfWeek.SUNDAY) }
                weekends -> generateSequence(target.plusDays(1)) { it.plusDays(1) }
                    .first { it.dayOfWeek in setOf(DayOfWeek.SATURDAY, DayOfWeek.SUNDAY) }
                explicitDate && !daily -> return null
                else -> target.plusDays(1)
            }
        }
        ParsedTime(target.toInstant().toEpochMilli(), repeat)
    }.getOrNull()
}
